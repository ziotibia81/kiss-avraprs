...
DECLARE_ISR(ADC_vect)
{
	...
	if (hw_afsk_dac_isr) {
		...
		if (res & 0b00010000) PORTE |= BV(3); else PORTE &= ~BV(3);
		if (res & 0b00100000) PORTH |= BV(3); else PORTH &= ~BV(3);
		if (res & 0b01000000) PORTH |= BV(4); else PORTH &= ~BV(4);
                if (res & 0b10000000) PORTH |= BV(5); else PORTH &= ~BV(5);
	}
	else {
		PORTE &= ~BV(3);
                PORTH &= ~BV(3);
